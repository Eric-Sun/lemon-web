...
(function($) { // create private scope (inside you can use $ instead of jQuery)
				...
				jQuery(this).click(function (ev) {
					...
					if (false == jQuery.data($this.get(0), "simpleDatepicker").hasDatepicker) {
						...
						if (initialDate && dateRegEx.test(initialDate)) {
							chosendate = parseDate(initialDate);
						...
						jQuery(document.body).mousedown(function(ev){
							if (jQuery.data($this.get(0), "simpleDatepicker").hasDatepicker) {
								var width = $(datepicker).width()+x+5;
								var height = $(datepicker).height()+y+5;
								if(ev.clientX < x || ev.clientX > width || ev.clientY < y || ev.clientY > height) {
	...
	jQuery.fn.simpleDatepicker.formatOutput = function (dateObj) {
		...
		//var dated = new Date(dateObj.getFullYear() + "/" + (dateObj.getMonth() + 1) + "/" + dateObj.getDate());
